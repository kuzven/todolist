...
from bot.utils.api import get_tasks


async def tasks_command(message: types.Message):
    try:
        tasks = await get_tasks(message.from_user.id)
        print(f"[DEBUG] Полученные задачи: {tasks}")  # Логируем список задач
        ...
        tasks_text = "Ваши задачи:\n\n" + "\n".join(
            [f"- {task['title']} (Категория: {task['category']}, Дедлайн: {task['due_date']})" for task in tasks]
        )
